...
export class Service extends RestService {
  ...
  search(info) {
    ...
    if (query !== "") endpoint = `${serviceUri}/bySize?${query}`;

    return super.get(endpoint);
  }

  generateExcel(info) {
    var endpoint = `${serviceUri}/bySize/download?unit=${info.unit}&date=${info.date}`;
    ...
    if (query !== "") endpoint = `${serviceUri}/bySize/download?${query}`;
